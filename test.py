...
class LinearRegressionTestCase(unittest.TestCase):
    ...
    def test_matxRound(self):
        ...
        for decpts in range(10):
            ...
            mat = matrix.tolist()
            dec_true = [
                [Decimal(str(round(num, decpts))) for num in row] for
                row in mat]

            matxRound(mat, decpts)
            dec_test = [[Decimal(str(num)) for num in row] for row in
                        mat]
    ...
    def test_transpose(self):
        for _ in range(100):
            ...
            self.assertEqual(t.shape, (c, r),
                             "Expected shape{}, but got shape{}".format(
                                 (c, r), t.shape))
    ...
    def test_matxMultiply(self):
        ...
        for _ in range(100):
            ...
            self.assertEqual(dotProduct.shape, dp.shape,
                             'Wrong answer, expected shape{}, but got shape{}'.format(
                                 dotProduct.shape, dp.shape))
        ...
        with self.assertRaises(ValueError,
                               msg="Matrix A\'s column number doesn\'t equal to Matrix b\'s row number"):
    ...
    def test_augmentMatrix(self):
        ...
        for _ in range(50):
            ...
            self.assertTrue(A.tolist() == Amat,
                            "Matrix A shouldn't be modified")
        ...
        for _ in range(9999):
            ...
            if np.linalg.matrix_rank(A) < r:
                ...
                self.assertEqual(np.array(x).shape, (r, 1),
                                 "Expected shape({},1), but got shape{}".format(
                                     r, np.array(x).shape))
